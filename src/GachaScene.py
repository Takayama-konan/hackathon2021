...
def gacha_description_run(gacha_name: str):
    ...
    DrawDisplay.initialDiplay([f"■□ お食事券: {ticket_number}枚"])  # お食事券

    command_number = 0
    command_line = [
        "1回引く (お食事券を1枚消費します)",
        "10+1回引く(お食事券を10枚消費します)",
        "もどる",
    ]
    ...
    while True:
        ...
        #コマンド操作#
        if key in CommandManager.ENTER:
            if command_line[command_number] == "1回引く (お食事券を1枚消費します)":
        ...
        DrawDisplay.initialDiplay([f"■□ お食事券: {ticket_number}枚"])  # お食事券
        if none_ticket_flag:  # お食事券がないとき
            DrawDisplay.initialDiplay(["お食事券が足りません!!"])
